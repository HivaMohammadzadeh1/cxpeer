...
The floor matters too. A Codex session with my usual MCP servers starts around 15k tokens; started with `-c mcp_servers={}`, a trivial turn costs 3.7k. A Claude session started with `--strict-mcp-config --mcp-config "" --disable-slash-commands` still registers as a peer, while `--bare` does not. `cxpeer spawn --lean` uses those flags, so a worker peer starts at the floor and the orchestrating session keeps only the summaries.
